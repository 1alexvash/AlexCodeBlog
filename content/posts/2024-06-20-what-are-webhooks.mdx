...
When everything goes well on the Stripe side, and the payment is confirmed, it issues an event, saying `'payment_intent.succeeded'` ✅ and forward this event via 🪝 webhook technology, basically forwarding this event from the stripe webhook URL `https://stripe.com/webhooks` to your web server URL `https://yourserver.com/api/buyItem` where you can securely reward the user with the item he has purchased. In case the payment did not go well, you're going to receive `'payment_intent.payment_failed'` ❌ and can send an according message to the user.
